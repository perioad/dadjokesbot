...
export const saveExplanationRequest = async (chatId, jokeId) => {
  try {
    ...
    const updationParams = {
      ...
      UpdateExpression:
        'SET explanation_requests = list_append(if_not_exists(explanation_requests, :emptyList), :newItem) ADD explanations_count :explanationsCount',
      ExpressionAttributeValues: {
        ':newItem': {
          L: [
            {
...
export const saveFeedback = async (chatId, feedback) => {
  try {
    ...
    const updationParams = {
      ...
      UpdateExpression:
        'SET feedbacks = list_append(if_not_exists(feedbacks, :emptyList), :newItem)',
      ExpressionAttributeValues: {
        ':emptyList': { L: [] },
        ':newItem': { L: [{ S: feedback }] },
      },
    };

    await client.send(new UpdateItemCommand(updationParams));

    log('success saving feedback');
  } catch (error) {
    handleError('saveFeedback', error);
  }
};
